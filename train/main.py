...
class Runner:
    def __init__(self, config):
        ...
        config.action_dim = self.envs.single_action_space.shape[0]  # 确保动作空间维度匹配
    ...
    def switch_map(self):
        ...
        self.num_envs = len(vector_maps)
        print("num_envs={}".format(self.num_envs))
        self.agent.reset_buffer(num_envs=self.num_envs)  # 重置缓冲区
    ...
    def evaluate_policy(self):
        # 评估策略
        self.evaluate_num += 1
        for map_index, env in enumerate(self.env_evaluate):  # 同时在所有已体验的关卡上评估策略
...
if __name__ == '__main__':
    config = get_configs()  # 获取配置
    config.algorithm = "CCPPO"

    config_dict = vars(config)  # 打印所有超参数
    for key in config_dict.keys():
        print("{}={}".format(key, config_dict[key]))

    runner = Runner(config=config)  # 创建Runner实例
    runner.run()  # 运行强化学习训练过程
